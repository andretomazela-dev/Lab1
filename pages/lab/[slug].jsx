    ...
    return (
      <div>
        ...
        <main className="container py-16">
          ...
          <p className="mt-4">
            Voltar para o{" "}
            <Link className="underline text-orange-600" href="/lab">
              LAB
            </Link>
            .
          </p>
    ...
    <div>
      ...
      <main className="bg-white">
        <section className="container pt-10 md:pt-14">
          ...
          {(post.tags || []).length ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {post.tags.map((tg) => (
                <Link
                  key={tg}
                  href={`/lab?tag=${encodeURIComponent(tg)}`}
          ...
          {/* HERO IMG — evita cortar cabeças */}
          <div className="mt-6 rounded-2xl overflow-hidden shadow-card relative h-[300px] sm:h-[360px] md:h-[460px]">
            <Image
              src={post.cover}
              alt={post.title}
              fill
        ...
        <section className="container py-10 md:py-14">
          <article
            className="prose prose-zinc max-w-none prose-a:text-orange-600"
          ...
          <section className="container pb-14 md:pb-20">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {related.map((p) => (
                <article
                  key={p.slug}
                  className="rounded-2xl overflow-hidden shadow-card bg-white ring-1 ring-black/5 hover:shadow-lg transition"
                >
...
export async function getStaticProps({ params }) {
  const post =
    labPosts.find((p) => p.slug === params.slug) || null;
